...
//Funciones de las consultas V1
void ReiniciarRespuesta(char respuesta[700]) {
...
void  DameListadoPartidasJugador(char respuesta[700],char nombre[50],MYSQL *conn) {
	MYSQL_RES *resultado;
	MYSQL_ROW row;
	char consulta[700];
...
void  DameIDDuracionGanador(char respuesta[700],char nombre[50],MYSQL *conn) {
	MYSQL_RES *resultado;
	MYSQL_ROW row;
	char consulta[700];
...
void  TiempoTotalJugador(char respuesta[700],char nombre[50],MYSQL *conn) {
	MYSQL_RES *resultado;
	MYSQL_ROW row;
	int err;
	char consulta[700];
...
void  DamePartidasGanadas(char respuesta[700],char nombre[50],MYSQL *conn) {
	MYSQL_RES *resultado;
	MYSQL_ROW row;
	char consulta[700];
	int err;
...
void *AtenderCliente(void *socket)
{
	...
	while (terminar==0)
	{
		...
		{
			ReiniciarRespuesta(respuesta);
			DameListadoPartidasJugador(respuesta,nombre,conn);
		}
		...
		{
			ReiniciarRespuesta(respuesta);
			DameIDDuracionGanador(respuesta,nombre,conn);
		}
		...
		{
			ReiniciarRespuesta(respuesta);
			TiempoTotalJugador(respuesta,nombre,conn);
		}
		...
		{
			ReiniciarRespuesta(respuesta);
			DamePartidasGanadas(respuesta,nombre,conn);
		}
		...
		else if (codigo==6)
		{
			...
			row = mysql_fetch_row (resultado);
			if (row == NULL) {
				strcpy(respuesta,"El usuario no esta registrado o has introducido mal los datos");
